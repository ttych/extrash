...
file_mon()
(
    file_mon__usage="fmon [-d] [-F] [-c command] [-p] [-s X] [-P prune] path1 path2 path..."
    file_mon__file=TRUE
    ...
    OPTIND=1
    while getopts :hFdc:ps:S:P: opt; do
	    case $opt in
            ...
            P) file_mon__prune="${file_mon__prune:+$file_mon__prune -o }-name $OPTARG -prune"
	    esac
    done
    shift $(($OPTIND - 1))
    ...
    while true; do
        for file_mon__p; do
            for file_mon__p_f in $(find "$file_mon__p" ${file_mon__prune:+$file_mon__prune -o} -print); do
